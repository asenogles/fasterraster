...
class Bil:
    ...
    def read_hdr(self, hname):
        """read the bil header file
        Args:
    ...
    def read_bil(self, fname):
        with open(fname, 'rb') as f:
            #buffer = f.read()
            #array = np.frombuffer(buffer, self.NPTYPE)
            array = np.fromfile(f, self.NPTYPE, count = self.NROWS * self.NCOLS)
        return array.reshape(self.NROWS, self.NCOLS)#, self.NBANDS) #TODO: fix for multiple bands - see ESRI Doc for order

    def write_bil(self, data, fname):
